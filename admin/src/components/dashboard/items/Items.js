...
export const Items=()=>{
    ...
    const toggleVerifyItem=async({ userId, itemId })=>{
        try{
            const response=await fetch(`${apiUrl}/admin/toggleVerifyItem/${userId}/${itemId}`, {
                method: "PUT",
                credentials: "include"
            });
            const result=await response.json();
                    ...
                    <div className="items-details">
                            ...
                            <tbody>
                                {items.map(item=>(
                                    <tr key={item._id} onClick={()=>fetchItem(item._id)}>
                                        ...
                                        <td><button className={item.verified ? "remove" : "verify"} onClick={()=>toggleVerifyItem({ userId: item.userId, itemId: item._id })}>
                        ...
                        {item ? (
                            <div className="item-details">
                                ...
                                <p>{item.type}</p>
                                <p>{item.pieceLeft}</p>
                                <button className={item.verified ? "remove" : "verify"} onClick={()=>toggleVerifyItem({ userId: item.userId, itemId: item._id })}>
                                    {item.verified ? "Remove" : "Verify"}
                                </button>
                            </div>
                        ):(
